...
import { makeSessionRoute } from './routes/sessionRoute.mjs';
import { makeUiRoute } from './routes/uiRoute.mjs';
...
const routes = {
  ...
  webhook: makeWebhookRoute({ installStore, seenEventStore, restomenumBase: config.restomenumBase }),
  action: makeActionRoute({ installStore }),
  session: makeSessionRoute({ installStore, pluginId: config.clientId }),
  ui: makeUiRoute(),
};
