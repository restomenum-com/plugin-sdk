  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      switch (route) {
        ...
        case 'GET /api/me':
          return await routes.session(req, res, url);
        case 'GET /ui':
          return routes.ui(req, res, url);
        case 'GET /':
...
function homeHtml() {
  return `<h2>Restomenum örnek eklenti</h2>
    <p>Uçlar: <code>/connect</code> (Connect redirect hedefi), <code>/webhook</code>, <code>/action</code>,
    <code>/ui</code> (Custom UI iframe sayfası), <code>/api/me</code> (session token doğrulama).</p>
